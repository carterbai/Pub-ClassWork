...
### 2.使用crackmapexec這個工具來對smb進行使用者名稱的列舉
...
### 6.利用crackmapexec加上user.txt對mssql進行測試
...
### 8.利用EXEC xp_dirtree 'C:\inetpub\wwwroot', 1, 1;指令查詢資料庫的檔案，找到一個備份的壓縮檔
#### xp_dirtree：擴展存儲過程的名稱。
#### 'C:\inetpub\wwwroot'：指定的目錄路徑，這裡是C:\inetpub\wwwroot。
#### 1：表示包含子目錄。
#### 1：表示返回完整的路徑。
![image](https://github.com/TwMoonBear-Arsenal/Pub-ClassWork/assets/144238471/b79d6427-c6a6-4735-a878-fe2b823ea167)
### 9.下載壓縮檔，打開它，發現內除了常見的html檔外，還有一個xml檔
